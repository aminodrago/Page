	...
	<div class='row view-toolbar'>
		...
		<div class="col-md-6 col-xs-8 view-search">
			<form class="form-horizontal" action="{{ URL::to('admin/page') }}" method="get">
				...
				<div class="input-group">
					<input type="search" class="form-control" name="q" value="{{$q}}"  placeholder="{{  Lang::get('appl.search') }}">
		...
		<div class="col-md-2 col-xs-4 view-buttons">
			<a class="btn btn-info pull-right {{ ($permissions['create']) ? '' : 'disabled' }} view-btn-create" href="{{ URL::to('admin/page/create') }}">
				<i class="fa fa-plus-circle"></i> {{ Lang::get('appl.new') }} {{ Lang::get('page::module.name') }}
			</a>
		</div>
	</div>
	...
	<div class='view-content'> 
		<table class="table">
			<thead>
				...
				<th width="70">{{ Lang::get('appl.options') }}</th>
			</thead>
			<tbody>
				...
				<tr>
					...
					<td>
						<div class="btn-group  btn-group-xs"> 
							<a type="button" class="btn btn-info  {{ ($permissions['edit']) ? '' : 'disabled' }} view-btn-edit" href="{{ URL::to('admin/page')}}/{{$page->id}}/edit" title="{{ Lang::get('appl.update') }} Page"><i class="fa fa-pencil-square-o"></i></a> 
							<a type="button" class="btn btn-danger action_confirm  {{ ($permissions['delete']) ? '' : 'disabled' }} view-btn-delete" data-method="delete" href="{{ URL::to('admin/page') }}/{{ $page->id }}" title="{{ Lang::get('appl.delete') }} Page"><i class="fa fa-times-circle-o"></i></a> 
						</div>
					</td>
				</tr>
				@endforeach
